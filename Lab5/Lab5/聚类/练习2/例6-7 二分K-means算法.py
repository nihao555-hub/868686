# -*- coding: utf-8 -*-
"""
Created on Mon May 18 21:48:24 2020
...
def randCent(dataSet,k):
    #随机生成k个点作为质心，全部质心都处于整个数据集的数据边界之内
    n=dataSet.shape[1] #获取数据集的维度
...
def biKmeans(dataSet, k, distMeas=distEclud):
    ...
    centList = [centroid0] #创建一个包含一个质心的列表
    #计算每个点到质心的误差值
    for j in range(m):
        clusterAssment[j,1] = distMeas(np.asmatrix(centroid0),dataSet[j,:])**2
    while (len(centList) < k):
        ...
        for i in range(len(centList)):
            ...
            sseNotSplit = sum(clusterAssment[np.nonzero(clusterAssment[:,0].A != i)[0],1]) #获取剩余数据集的SSE
...
if __name__ == '__main__':
    ...
    print("用二分K-Means算法原理聚类耗时：", time.time() - s,"秒")
    ...
    plt.scatter(centroids[:, 0], centroids[:, 1], c='w', s=100, alpha=0.5)
